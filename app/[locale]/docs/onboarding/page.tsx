import {
  H1, H2, H3, P, Ul, Ol, Li, CheckList, CheckItem,
  Blockquote, Hr, Strong, A, InlineCode, CodeBlock,
...
function FrContent({ locale }: { locale: string }) {
  return (
    <>
      <H1>👋 Bienvenue</H1>
      <P>Ce document est ton point de départ. Lis-le entièrement avant d&apos;écrire la moindre ligne de code.</P>

      <H2>📚 Documents à lire dans l&apos;ordre</H2>
      <Ol>
        <Li><A href={`/${locale}/docs/gitflow`}>Notre GitFlow</A> — Comment on gère les branches</Li>
        <Li><A href={`/${locale}/docs/commits`}>Conventions de commits</A> — Comment on nomme les commits</Li>
        <Li><A href={`/${locale}/docs/pr`}>Process de Pull Request</A> — Comment on soumet son travail</Li>
        <Li><A href={`/${locale}/docs/changesets`}>Changesets</A> — Comment on versionne le code et génère les changelogs</Li>
      </Ol>
...
cd nom-du-repo`}</CodeBlock>
      ...
      <CheckList>
        ...
        <CheckItem>Tu as créé ta branche depuis <InlineCode>develop</InlineCode> (jamais depuis <InlineCode>main</InlineCode>)</CheckItem>
      </CheckList>
    </>
  );
}
...
function EnContent({ locale }: { locale: string }) {
  return (
    <>
      <H1>👋 Welcome</H1>
      <P>This document is your starting point. Read it entirely before writing a single line of code.</P>

      <H2>📚 Documents to read in order</H2>
      <Ol>
        <Li><A href={`/${locale}/docs/gitflow`}>Our GitFlow</A> — How we manage branches</Li>
        <Li><A href={`/${locale}/docs/commits`}>Commit Conventions</A> — How we name commits</Li>
        <Li><A href={`/${locale}/docs/pr`}>Pull Request Process</A> — How we submit our work</Li>
        <Li><A href={`/${locale}/docs/changesets`}>Changesets</A> — How we version code and generate changelogs</Li>
      </Ol>
...
cd repo-name`}</CodeBlock>
      ...
      <CheckList>
        ...
        <CheckItem>You&apos;ve created your branch from <InlineCode>develop</InlineCode> (never from <InlineCode>main</InlineCode>)</CheckItem>
      </CheckList>
    </>
  );
}
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return locale === "fr" ? <FrContent locale={locale} /> : <EnContent locale={locale} />;
}
